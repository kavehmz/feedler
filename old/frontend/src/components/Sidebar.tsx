...
function FeedRow({
  ...
  const handleRename = async () => {
    ...
    if (next == null || next.trim() === '' || next === feed.title) return
      ...
      <div className="bg-white dark:bg-ink-900 rounded-lg shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3 text-sm">
          <div>
            ...
            <a href={feed.xml_url} target="_blank" rel="noopener noreferrer" className="text-xs text-accent-600 hover:underline break-all">
              {feed.xml_url}
            </a>
          </div>
          <div>
