...
func (s *Server) listArticles(w http.ResponseWriter, r *http.Request) {
	...
	listQ := `SELECT a.id, a.feed_id, f.title, COALESCE(f.folder,''),
		COALESCE(a.title,''), COALESCE(a.link,''), COALESCE(a.author,''),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.FeedID, &a.FeedTitle, &a.FeedFolder,
			&a.Title, &a.Link, &a.Author, &a.Summary, &pub, &a.FetchedAt,
...
func (s *Server) getArticle(w http.ResponseWriter, r *http.Request) {
	...
	err = s.DB.QueryRowContext(r.Context(), `
		...
		Scan(&a.ID, &a.FeedID, &a.FeedTitle, &a.FeedFolder,
			&a.Title, &a.Link, &a.Author, &a.Summary, &a.Content, &fullc,
...
func (s *Server) markRead(w http.ResponseWriter, r *http.Request) {
	...
	_, err := s.DB.ExecContext(r.Context(), `UPDATE articles SET is_read = 1 WHERE id = ?`, id)
	if err != nil {
		writeErr(w, 500, err.Error())
		return
	}
...
func (s *Server) markUnread(w http.ResponseWriter, r *http.Request) {
	id, _ := strconv.ParseInt(chi.URLParam(r, "id"), 10, 64)
	_, err := s.DB.ExecContext(r.Context(), `UPDATE articles SET is_read = 0 WHERE id = ?`, id)
	if err != nil {
		writeErr(w, 500, err.Error())
		return
	}
...
func (s *Server) toggleStar(w http.ResponseWriter, r *http.Request) {
	...
	_, err := s.DB.ExecContext(r.Context(),
		`UPDATE articles SET is_starred = CASE is_starred WHEN 1 THEN 0 ELSE 1 END WHERE id = ?`, id)
	if err != nil {
		writeErr(w, 500, err.Error())
		return
	}
