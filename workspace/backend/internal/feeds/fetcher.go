...
func (f *Fetcher) refreshOne(ctx context.Context, feedID int64, url, etag, lastModified string) (int, error) {
	...
	if resp.StatusCode == http.StatusNotModified {
		_, _ = f.DB.Exec(`UPDATE feeds SET last_fetched_at = ?, last_error = NULL WHERE id = ?`, time.Now().UTC(), feedID)
		return 0, nil
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
	...
	_, _ = f.DB.Exec(
		...
		newEtag, newLM, time.Now().UTC(), feedID,
	)
	return count, nil
}
...
func (f *Fetcher) recordError(feedID int64, msg string) {
	...
	_, _ = f.DB.Exec(`UPDATE feeds SET last_fetched_at = ?, last_error = ? WHERE id = ?`, time.Now().UTC(), msg, feedID)
